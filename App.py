...
with tab5:
    ...
    with sub_tab1:
        ...
        conn = sqlite3.connect("database/agis_database.db")
        df_usuarios = pd.read_sql_query("SELECT id, nombre, username, perfil, telefono FROM usuarios", conn)
        conn.close()
    ...
    with sub_tab2:
        ...
        chacra_sel = st.text_input("Nombre de la Chacra (debe ser la misma que la DB):", key="in_chacra_carga")
        ...
        if st.button("Procesar y Asignar"):
            if user_sel and chacra_sel and archivo:
                # Directorio dinámico: uploads/usuario/chacra/
                ...
                st.success(f"Archivo {archivo.name} guardado en servidor para {user_sel}.")
                st.balloons()
            else:
